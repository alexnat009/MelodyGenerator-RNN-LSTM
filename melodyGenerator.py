...
class MelodyGenerator:
    ...
    def save_melody(self, melody, step_duration=0.25, format="midi", file_name="generated_melody.midi", save_to_file=False):
        # create a music21 stream
        stream = m21.stream.Stream()

        # parse all the symbols in the melody and create note/reset objects
        start_symbol = None
        step_counter = 1

        for i, symbol in enumerate(melody):
            # handle case: note/rest
...
if __name__ == "__main__":
    mg = MelodyGenerator()
    seed = "62 _ _ 62 _ _ 60 _ _ _ 50 _ _ 66 _ _ 66 _ _ 67 _ _ _"
    melody = mg.generate_melody(seed, 500, SEQUENCE_LENGTH, 0.7)
    melody = mg.save_melody(melody)
    melody.show()